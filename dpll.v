...
Ltac wf :=
  ...
  repeat match goal with
           ...
           | |- wf_clause _ ?c => progress unfold c; red; intros
           | H: _ \/ _ |- _ => destruct H
...
Lemma getVal_nth : forall m n, n < length m -> getVal m n = Some (nth n m true).
...
Lemma getVal_m2i : forall m n, n < length m -> getVal m n = Some (m2i m n).
  unfold m2i; apply getVal_nth; auto.
Qed.
...
Theorem dpll_Some_sound : forall F m nv, dpll nv F = Some m -> wf_formula nv F
                                         -> satisfiable F.
  red; intros.
  exists (m2i m); eapply okFormula_interpFormula; try eassumption; eauto 3.
Qed.
...
Lemma okLit_app : forall b m x, okLit m x = false
  ...
  destruct (le_lt_dec (length m) (litVar x)).
  rewrite getVal_None in * by auto; congruence.
  rewrite getVal_nth in * by (autorewrite with list; omega).
